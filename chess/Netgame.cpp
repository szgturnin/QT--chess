...
void NetGame::slotNewConnection()
{
    ...
    _socket = _server->nextPendingConnection();//连接信号newConnection，在槽函数里调用nextPendingConnection获取连接进来的socket。
...
{
    QByteArray buf = _socket->readAll();//读取设备中所有的数据，并将其作为一个字节数组返回，可能返回一个空的QByteArray
    switch (buf.at(0)) {//at()返回索引位置的i的字符
    case 1:
        initFromNetwork(buf);
        break;
    case 2:
