...
Step* SingleGame::getBestMove()//预测最好的步数，博弈算法
{
    Step* ret = NULL;
    QVector<Step*> steps;
...
int SingleGame::getMinScore(int level, int curMin)
{
    ...
    while(steps.count())
    {
        Step* step = steps.last();//返回对向量中最后一个项的引用
        steps.removeLast();
...
int SingleGame::getMaxScore(int level, int curMax)
{
    ...
    while(steps.count())//分析每种steps，得到一个最优的step。2步预测计算
    {
        Step* step = steps.last();
        steps.removeLast();
...
void SingleGame::fakeMove(Step *step)//假装移动棋子
{
...
void SingleGame::getAllPossibleMove(QVector<Step *> &steps)//得到所有可能的下子可能情况
{
    int min, max;
    if(this->_bRedTurn)
    {
